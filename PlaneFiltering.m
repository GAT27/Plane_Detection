function [P,R,C,O,plane] = PlaneFiltering(I,aspect,k_max,l,eta,S,epsilon,a_in)
%Takes in a depth map I
%n_max      =   Maximum total number of 3D points
%aspect     =   Aspect ratio
...
while (k < k_max)
    ...
    %Choose 3 random points on depth map (making sure it is within bounds,
    %seems to mostly focus around the middle, but that is okay since we
    %only care about the center
    %d0 = [randi([0,w-1]),randi([0,h-1])];
    ...
    %Projection parameter, may change once projector is gotten, so these
    %are rough estimates based on the kinect
    ...
    if (inliers > (a_in * l))
        ...
        %Build up a plane based on the relative points that were collected,
        %will be used to find middle to better approximate planes
        p_avr = (1/size(P_hat,1)) * sum(P_hat);
        %SM = [0,0,0;0,0,0;0,0,0];
        %SM = double(SM);
        %for ind=1:size(P_hat,1)
        %    display(1);
        %    display(P_hat(ind,:));
        %    display(p_avr);
        %    SM = (P_hat(ind,:)-p_avr).' * (P_hat(ind,:)-p_avr) + SM;
        %end
        %[V,D] = eig(SM);
        %normal = find(min(sum(D)));
        %if normal==1
        %    b1 = V(:,2);
        %    b2 = V(:,3);
        %elseif normal==2
